...
Excel Liteは、Webブラウザ上で動作する軽量なスプレッドシートアプリケーションです。ユーザーはグリッド状のセルに値や数式を入力でき、リアルタイムで計算結果が反映されます。本システムは計算ロジック（Spreadsheet Engine）とUIコンポーネントを明確に分離し、ロジック部分は徹底的なテストカバレッジを実現します。

## 用語集

- **システム**: Excel Liteアプリケーション全体
